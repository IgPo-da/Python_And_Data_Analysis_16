...
names = ['Sarah', 'Jessica', 'Anthony', 'Jack', 'Simon', 'Arthur', 'Maria',
         'Samantha']
long_names = []
short_names = []
print()
...
# Given a list where each element is a year. Determine whether the given
...
# Write a program that prompts the user for a string and checks if the string
#  contains only unique characters.
user_input = input('\nEnter something: ')

unic = len(user_input) == len(set(user_input))
print(f"The string you provided contains 
      {'duplicated' if not unic else 'only unique'} 
      characters {set(user_input) if unic else ''}\n")
...
# If number is divided by 5 and by 3 without a remainder print number
#  and FIZZBUZZ

# I found a hint on Google, but wrote it myself :)
# I was surprised that I could use string concatenation for this particular
#  case. 
# I was going to use another comparison on 3 and 5 at the same time or another
#  2 comparisons within the checks on 3 and 5
