...
def get_rob():
    rob_number = int( id_code[-4:-1])
    #print("place of birth number is - ", number)
    place_found = "False"
    for line in pob.splitlines():
        line = line.strip()
        if not line:
            continue
        numbers_range, place = line.split("=")
        a, b = numbers_range.split("...")
        print(int(a), int(b))
        if rob_number >= int(a) and rob_number <= int(b):
            place_found = "True"
            print("You`re born in :", place.strip())
            break
